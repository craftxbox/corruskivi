export function preprocessChange(dialogue: string): string {
    let execs = dialogue.match(/THEN::(.+)$/gm);
    ...
    dialogue = dialogue.replaceAll(/^ +THEN::[ ;]*$\n/gm, ""); // clean up any empty exec lines
    return dialogue;
}
...
export function postprocessChange(dialogue: string): string {
    ...
    for (let code of codes) {
        ...
        let regex = new RegExp(`THEN::(.+)\n +${code.replaceAll("[", "\\x5b").replaceAll("]", "\\x5d").replaceAll(".", "\\x2e")}`, "m");

        if (dialogue.match(regex)) {
            let match = dialogue.match(regex);
            if (!match) throw new Error("Failed to match code in dialogue. This condition should be impossible.");
            dialogue = dialogue.replace(match[0], `THEN::${match[1]};${newCode}`);
        } else {
            dialogue = dialogue.replace(code, `THEN::${newCode}`);
        }
    }
    return dialogue;
}

let oldChangeBranch = window.changeBranch;
let oldChangeDialogue = window.changeDialogue;

window.changeBranch = function (name: string) {
    oldChangeBranch(name);
    document.querySelectorAll(".dialogue-message:not(.sent)").forEach(el => el.classList.add("sent"));
}

window.changeDialogue = function (name: string) {
    oldChangeDialogue(name);
    document.querySelectorAll(".dialogue-message:not(.sent)").forEach(el => el.classList.add("sent"));
}